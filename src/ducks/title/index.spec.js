...
test('ducks/title/reducer', t => {
	...
	t.deepEqual(
		reducer(undefined, {type: constants.UPDATE, value: 'Title'}),
		'Title',
		'Must handle constants.UPDATE action.'
	);

	t.deepEqual(
		reducer('Default', {type: constants.UPDATE, value: 'Title'}),
		'Title',
		'Must handle constants.UPDATE action with defined initialState.'
	);
});

test('ducks/title/actions/update', t => {
